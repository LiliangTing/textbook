					...
					<div class="col-md-12">
						...
						<c:if test="${!empty page.list}">
							...
							<table class="table table-hover" id="sample-table-1">
								...
								<tbody>
									<c:forEach items="${page.list}" var="courSem">
										...
										<tr>
											...
											<td class="center"><c:if test="${courSem.book==null}">
													<span class="label label-sm label-danger">异常</span>
												</c:if> <c:if test="${courSem.book!=null}">
													<c:if test="${courSem.book.discount!=0.00 }">
														<span class="label label-sm label-success">正常</span>
													</c:if>
													<c:if test="${courSem.book.discount == 0}">
														<a href="admin/toSetBookDiscount/${courSem.book.isbn13}">设置价格</a>
													</c:if>
												</c:if></td>
										</tr>
									</c:forEach>
									<tr>
										<td colspan="10" align="right" style="">
											...
											<div>
												<ul class="pagination">
													<li><a href="admin/viewBook/${page.prePage}">&laquo;上一页</a></li>

													<c:forEach items="${page.navigatepageNums }" var="p">
														<c:choose>
															<c:when test="${p == page.pageNum }">
																<li class="active"><a href="admin/viewBook/${p}">
																		${p} </a></li>
															</c:when>
															<c:otherwise>
																<li><a href="admin/viewBook/${p}"> ${p} </a></li>
															</c:otherwise>
														</c:choose>


													</c:forEach>
													<li><a href="admin/viewBook/${page.nextPage}">下一页&raquo;</a></li>
